...
namespace ApiSapScada.Controllers
{
    ...
    public class RecipeController : BaseApiController
    {
        ...
        public async Task<IActionResult> Created([FromBody] EventPayload<RecipeData> payload)
        {
            CreateCommand<RecipeData> result = new()
            {
        ...
        [ProducesResponseType(typeof(List<Notify>), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(List<Notify>), StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Changed([FromBody] EventPayload<RecipeData> payload)
        {
            UpdateCommand<RecipeData> result = new()
            {
                EventPayload = payload
            };
            return Result(await Mediator.Send(result));
