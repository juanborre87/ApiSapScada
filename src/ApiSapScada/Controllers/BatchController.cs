...
namespace ApiSapScada.Controllers
{
    ...
    public class BatchController : BaseApiController
    {
        ...
        public async Task<IActionResult> Created([FromBody] EventPayload<BatchData> payload)
        {
            CreateCommand<BatchData> result = new()
            {
        ...
        [ProducesResponseType(typeof(List<Notify>), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(List<Notify>), StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Changed([FromBody] EventPayload<BatchData> payload)
        {
            UpdateCommand<BatchData> result = new()
            {
                EventPayload = payload
            };
            return Result(await Mediator.Send(result));
